...
func TestOpenAIGatewayHandlerImages_DisabledAPIKeyRejectsBeforeScheduling(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.Equal(t, http.StatusForbidden, rec.Code)
	require.Equal(t, "permission_error", gjson.GetBytes(rec.Body.Bytes(), "error.type").String())
	require.Contains(t, rec.Body.String(), service.APIKeyImageGenerationPermissionMessage())
}
