...
func TestForwardAsAnthropicWithDisplayModel_HidesMappedNonStreamingModel(t *testing.T) {
	t.Parallel()
	gin.SetMode(gin.TestMode)
	...
	upstreamBody := strings.Join([]string{
		`data: {"type":"response.completed","response":{"id":"resp_display","object":"response","model":"gpt-5.4","status":"completed","output":[{"type":"message","id":"msg_1","role":"assistant","status":"completed","content":[{"type":"output_text","text":"ok"}]}],"usage":{"input_tokens":5,"output_tokens":2,"total_tokens":7}}}`,
		"",
		"data: [DONE]",
		"",
	}, "\n")
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_display"}},
		Body:       io.NopCloser(strings.NewReader(upstreamBody)),
	}}
	...
	result, err := svc.ForwardAsAnthropicWithDisplayModel(context.Background(), c, account, body, "", "gpt-5.4", OpenAIMessagesDispatchFableModel)
	require.NoError(t, err)
	require.NotNil(t, result)
	require.Equal(t, OpenAIMessagesDispatchFableModel, result.Model)
	require.Equal(t, "gpt-5.4", result.BillingModel)
	require.Equal(t, "gpt-5.4", result.UpstreamModel)
	require.Equal(t, "gpt-5.4", gjson.GetBytes(upstream.lastBody, "model").String())
	require.Equal(t, OpenAIMessagesDispatchFableModel, gjson.GetBytes(rec.Body.Bytes(), "model").String())
	require.NotContains(t, rec.Body.String(), `"model":"gpt-5.4"`)
}

func TestForwardAsAnthropicWithDisplayModel_HidesMappedStreamingModel(t *testing.T) {
	t.Parallel()
	gin.SetMode(gin.TestMode)
	...
	upstreamBody := strings.Join([]string{
		`data: {"type":"response.created","response":{"id":"resp_display_stream","model":"gpt-5.4","status":"in_progress","output":[]}}`,
		"",
		`data: {"type":"response.output_text.delta","delta":"ok"}`,
		"",
		`data: {"type":"response.completed","response":{"id":"resp_display_stream","object":"response","model":"gpt-5.4","status":"completed","output":[{"type":"message","id":"msg_1","role":"assistant","status":"completed","content":[{"type":"output_text","text":"ok"}]}],"usage":{"input_tokens":5,"output_tokens":2,"total_tokens":7}}}`,
		"",
		"data: [DONE]",
		"",
	}, "\n")
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_display_stream"}},
		Body:       io.NopCloser(strings.NewReader(upstreamBody)),
	}}
	...
	result, err := svc.ForwardAsAnthropicWithDisplayModel(context.Background(), c, account, body, "", "gpt-5.4", "claude-opus-4-8")
	require.NoError(t, err)
	require.NotNil(t, result)
	...
	require.Equal(t, "gpt-5.4", result.UpstreamModel)
	require.Equal(t, "gpt-5.4", gjson.GetBytes(upstream.lastBody, "model").String())
	require.Contains(t, rec.Body.String(), `"model":"claude-opus-4-8"`)
	require.NotContains(t, rec.Body.String(), `"model":"gpt-5.4"`)
}
...
func TestForwardAsAnthropic_HTTPContextWindowErrorPromptsCompact(t *testing.T) {
	t.Parallel()
	gin.SetMode(gin.TestMode)

	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	body := []byte(`{"model":"claude-opus-5","max_tokens":16,"messages":[{"role":"user","content":"hello"}],"stream":false}`)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/messages", bytes.NewReader(body))
	c.Request.Header.Set("Content-Type", "application/json")

	upstreamErr := `{"error":{"message":"Your input exceeds the context window of this model.","type":"invalid_request_error","code":"context_length_exceeded"}}`
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusBadRequest,
		Header:     http.Header{"Content-Type": []string{"application/json"}, "x-request-id": []string{"rid_context_http"}},
		Body:       io.NopCloser(strings.NewReader(upstreamErr)),
	}}
	...
	require.Error(t, err)
	require.True(t, IsOpenAIContextWindowExceededError(err))
	require.Nil(t, result)
	require.Equal(t, http.StatusBadRequest, rec.Code)
	require.Equal(t, "invalid_request_error", gjson.GetBytes(rec.Body.Bytes(), "error.type").String())
	require.Equal(t, AnthropicContextWindowExceededClientMessage, gjson.GetBytes(rec.Body.Bytes(), "error.message").String())
	require.NotContains(t, rec.Body.String(), "gpt-")
	require.NotContains(t, rec.Body.String(), "OpenAI")
	require.NotContains(t, rec.Body.String(), "ChatGPT")
}
...
func TestForwardAsAnthropic_StreamEOFAfterPartialTextReturnsMissingTerminalError(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_partial_text_eof"}},
		Body:       io.NopCloser(strings.NewReader(upstreamBody)),
	}}
...
func TestForwardAsAnthropic_StreamEOFAfterOpenToolUseReturnsMissingTerminalError(t *testing.T) {
	gin.SetMode(gin.TestMode)

	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	body := []byte(`{"model":"gpt-5.4","max_tokens":16,"messages":[{"role":"user","content":"use tool"}],"tools":[{"name":"Bash","description":"run command","input_schema":{"type":"object","properties":{"command":{"type":"string"}}}}],"stream":true}`)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/messages", bytes.NewReader(body))
	c.Request.Header.Set("Content-Type", "application/json")

	upstreamBody := strings.Join([]string{
		`data: {"type":"response.created","response":{"id":"resp_tool_partial","model":"gpt-5.4"}}`,
		"",
		`data: {"type":"response.output_item.added","output_index":0,"item":{"type":"function_call","call_id":"call_bash","name":"Bash"}}`,
		"",
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_partial_tool_eof"}},
		Body:       io.NopCloser(strings.NewReader(upstreamBody)),
	}}
...
func TestForwardAsAnthropic_BufferedContextWindowErrorPromptsCompact(t *testing.T) {
	...
	upstreamBody := strings.Join([]string{
		`data: {"error":{"message":"Your input exceeds the context window of this model. Please adjust your input and try again.","type":"invalid_request_error"}}`,
		"",
	}, "\n")
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_context_window"}},
		Body:       io.NopCloser(strings.NewReader(upstreamBody)),
	}}
...
func TestForwardAsAnthropic_ResponseFailedBeforeOutputReturnsStreamError(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstreamBody := strings.Join([]string{
		...
		"",
		`data: {"type":"response.failed","response":{"id":"resp_failed","status":"failed","error":{"message":"upstream processing failed"},"usage":{"input_tokens":3,"output_tokens":0}}}`,
		"",
	}, "\n")
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_response_failed"}},
		Body:       io.NopCloser(strings.NewReader(upstreamBody)),
	}}
	...
	require.Contains(t, rec.Body.String(), "Upstream response failed")
	require.NotContains(t, rec.Body.String(), `"type":"message_stop"`)
}

func TestForwardAsAnthropic_StreamingContextWindowErrorPromptsCompact(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstreamBody := strings.Join([]string{
		`data: {"type":"response.created","response":{"id":"resp_context","model":"gpt-5.4"}}`,
		"",
		`data: {"type":"response.failed","response":{"id":"resp_context","status":"failed","error":{"message":"Your input exceeds the context window of this model."}}}`,
		"",
	}, "\n")
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_stream_context_window"}},
		Body:       io.NopCloser(strings.NewReader(upstreamBody)),
	}}
	...
	require.Contains(t, rec.Body.String(), "Run /compact")
	require.NotContains(t, rec.Body.String(), "Upstream response failed")
}
