...
func TestRedactClaudeOnlyModelStatsMapsGPTToClaudeAliases(t *testing.T) {
	...
	require.Len(t, got, 4)
	require.Equal(t, "claude-haiku-4-5-20251001", got[0].Model)
	require.Equal(t, int64(5), got[0].Requests)
	require.Equal(t, "claude-sonnet-4-6", got[1].Model)
	require.Equal(t, int64(4), got[1].Requests)
	require.Equal(t, "claude-opus-4-8", got[2].Model)
	require.Equal(t, int64(3), got[2].Requests)
	require.Equal(t, "claude-opus-4-7", got[3].Model)
	require.Equal(t, int64(3), got[3].Requests)
	require.Equal(t, int64(24), got[3].TotalTokens)
}
