...
- 如果上游把 `web_search` `<tool_call>` 伪装成普通 assistant text，且里面包含 continuation summary，则在转换层和最终 SSE 出口都抑制该文本块，避免 Claude Code UI 当成真实搜索进度展示。
...
- `TestStreamingWebSearchSuppressesUnsafeTextualToolCall`
