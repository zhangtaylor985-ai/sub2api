...
- 2026-05-27 线上用户反馈补充：`Searched:` 后出现 `This session is being continued from a previous conversation...`，说明 OpenAI `web_search_call` 没有 `action.query` 时，Sub2API 把 Claude Code resume/compact 的 continuation summary 当成 fallback search query 展示了。修复边界：fallback query 进入 state、CLI synthetic text、VSCode thinking 和 `server_tool_use.input.query` 时都必须经过同一套 search-query 清洗；命中 continuation summary 时改为 generic `Searching the web.` / `Searched the web.`，不得暴露会话摘要。
