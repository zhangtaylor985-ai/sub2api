...
# 2026-08-20 外部额度闸门策略重构

- 生产账号 `dangvanbinh10031983@gmail.com` 在 12:50:07 有一条本站 usage，12:55 UI 显示上游主窗口累计用量 1.0%，但最新状态已被覆盖为 `no_external_decrease`。
- 现实现按一分钟区间判断：只要区间内存在本站请求，就先写入 `local_traffic_detected` 并保存当前上游快照；下一轮因此可能只看到 `1% -> 1%`，外部信号不可回溯。
- 现实现放行租约仅 10 分钟，且状态只保存最新一次判断，无法从 UI 判断曾经是否放行及为何关闭。
- 用户不接受该策略，已确认采用“关闭状态确认外部消耗 + 固定放行 60 分钟 + 本站流量不续期 + 到期重新观察”。
- 关键安全约束：只有关闭状态的干净观察区间才能可靠归因；开放期间的任何上游增长均不能用于续租。
- UI 需要把“上游累计用量”和“本轮确认的外部增量”明确分开，并保留确认时间与固定截止时间。
- 现状态仅保存最新上游快照和活动 `lease_until`；租约结束后会丢失确认时间、历史截止时间和曾经放行的证据。
- 新状态需要同时保存：最新上游快照、关闭观察基线、观察可判定时间、最近一次外部确认时间、最近一次固定放行截止时间、当前活动截止时间，以及有界的最近状态事件。
- 关闭观察不能继续使用“相邻一分钟差值”；应在无本站流量时保留同一基线，让外部增量跨多个轮询累积，直到达到上游可观察精度。
- 租约期间必须优先保留原始固定截止时间且不查询本站 usage 续租；到期、窗口变化或上游不可用后重新建立观察基线。
- 为排除刚关闭时的在途请求，采用默认 2 分钟安静观察期；期间如发现本站请求，则从当前上游快照重新计时。
- 现有 JSON `extra` 可兼容扩展这些字段，不需要数据库 migration；旧状态缺少新基线字段时会安全地重新建立观察状态。
- 前端类型和详情面板都直接读取账号 `extra`，无需增加专用状态查询 API；现有启停与立即检查接口可保持不变。
- 当前详情面板只有累计用量、外部增量、最近检查、活动截止四项，适合扩展为观察基线、最近确认时间、最近放行截止与活动倒计时，并新增有界事件列表。
- 账号列表徽标只依赖 `schedulable` 与错误 decision，新状态可向后兼容；只需调整标题文案为“固定一小时放行”。
- UI 已选择显示三条固定策略标签、六项核心数据和最近12个状态变化；活动租约结束后仍显示最近一次确认时间与历史截止时间，避免再次出现“当前为什么关闭”无法回溯。
- 为防止脏状态绕过归因，闸门启用但账号处于可调度且没有有效租约时，会立即关闭并重新建立观察基线。
...
- 生产当前运行ARM64 systemd binary，健康且无重启；本次功能不涉及migration，发布只需应用binary替换和短暂服务重启。
- `origin/main` 已包含外部额度闸门基础调度与第一版UI，本分支仅追加固定时长升级及会话安全排空两次提交，因此可快进推送，无需合并远端漂移。
...
- 生产 canary 日志确认候选应用的数据库/业务时区初始化为 `Asia/Shanghai`，新路由未认证返回 401，解决了超级用户 `psql SHOW TimeZone=GMT` 与应用连接时区不同的证据边界。
- 生产发布后公网实际 `ApiKeysView` 与中文资源同时命中两个快捷按钮和新 API 路径；Chrome 登录态只读验收确认桌面与 720px 窄屏弹窗均可操作。
- API Key ID 525 在最初日用量清零后继续产生真实请求；发布后回读日/周用量约 31.60，窗口起点未被发布改变。近两小时 97 条 usage 说明这是后续流量，不是部署回滚或重置失败。

---

# 2026-08-24 API Key 叠加套餐计费链路调查

- 目标 Key 的认证快照已正确汇总 3 个生效双人车套餐为日 `$450`、周 `$1500`、并发 `9`；问题不在 UI 或套餐时间表。
- 根因是 `usage_billing_repo` 事务锁定状态仍只读 API Key 原始限额或分组 `$300/$1000`，未读 `api_key_plan_packages`；Redis 已记录真实请求用量，DB/UI 却在 `$300` 停止，导致展示与 403 不一致。
- 旧计费链路在原始日额度用完后继续将费用分配到历史 token package，使套餐日额度没有成为事务一致的主限额。
- 最佳实践是把“是否已由套餐管理”与“当前生效额度”分开：只要存在任意套餐行即进入 plan-managed，当前限额只汇总 `starts_at <= NOW() AND expires_at > NOW()` 的套餐；这样可在到期转换时自动剔除对应额度。
- 新计费 SQL 在同一事务里 lateral aggregate 套餐限额并锁定 API Key，生效套餐头寸优先于历史 token package，避免认证与计费采用两个事实源。
- 新增套餐原来只失效认证缓存；修复后同时失效 `apikey:rate:<id>`，保证购买后下一请求立即获得新额度。
- 生产保客桥接只覆盖目标 Key，有完整数据库备份、精确套餐聚合前置条件和缓存失效；正式修复发布后已恢复原始限额字段为 0，无永久特例。
- 恢复后客户高强度使用使日用量从约 `$300` 增长到 `$448`；北京时间 00:00 按既有规则正常重置，之后新日窗口从 0 增长，证明不是重复扣费。
- 本次不修改 Claude/OpenAI 协议转换、streaming、thinking、tool use 或 web search，因此 cc1 黑盒不在本次回归范围；生产使用记录和公网 API 为计费链路的直接验收证据。
