export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      externalQuotaGate: {
        title: '外部额度闸门',
        description: '仅检测到非本系统产生的上游额度消耗时，临时允许调度。',
        enable: '启用外部额度闸门',
        disable: '关闭外部额度闸门',
        available: '当前可用',
        unavailable: '当前不可用',
        ...
        externalDelta: '外部消耗增量',
        lastCheck: '最近检查',
        leaseUntil: '可用期截止',
        failClosedHint: '检查异常、没有外部消耗、检测到本系统流量或可用期结束时，账号都会保持不可用。',
        disabledHint: '闸门已关闭，账号保持不可调度；重新启用后会先建立基线，不会立即放行。',
        schedulableManaged: '该账号的可调度状态由外部额度闸门自动管理',
        badgeAvailable: '外额放行',
        badgeAvailableTitle: '检测到外部额度消耗，当前处于临时可用期',
