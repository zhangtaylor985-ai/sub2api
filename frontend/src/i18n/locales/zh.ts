export default {
  ...
  admin: {
    ...
    sessionDelivery: {
      title: 'Session 交付控制台',
      description: '监控从网关捕获、隔离数据库、小时归档到 Google Drive 校验的完整链路，并管理全局与 API Key 级采集策略。',
      refresh: '刷新状态',
      refreshing: '正在刷新',
      lastObserved: '最近观测',
      loadFailed: 'Session 交付状态加载失败',
      attentionRequired: '交付链路需要关注',
      notYet: '尚未归档',
      deliverable: '可交付',
      archivedRecords: '已归档记录',
      fullReadbackVerified: '上传完成并通过完整回读 SHA 校验',
      status: {
        healthy: '运行正常',
        degraded: '部分降级',
        critical: '严重告警',
      ...
      batches: {
        ...
        waitingDescription: '没有归档批次并不代表异常；当前小时封账后才会生成交付文件。'
      },
      batchStatus: {
        exporting: '导出中',
        archived: '已上传待验证',
      ...
      warnings: {
        policy_summary_unavailable: '采集策略汇总暂不可用',
        spool_unavailable: '网关缓冲区不可读',
        spool_usage_critical: '网关缓冲区使用率达到 80%',
        spool_usage_high: '网关缓冲区使用率达到 60%',
        spool_quarantine_present: '缓冲区存在隔离记录',
        spool_forwarding_delayed: '待发送记录已积压超过 15 分钟',
        remote_status_not_configured: '隔离数据库状态通道未配置',
        remote_status_unavailable: '隔离数据库状态通道不可用',
        disk_guard_reached: '数据库服务器磁盘保护阈值已触发',
        disk_usage_high: '数据库服务器磁盘使用率偏高',
        archive_batches_failed: '存在失败的归档批次',
        archive_in_progress: '归档任务正在执行'
      },
      policy: {
        title: 'Session 采集管理',
        description: '全局模式决定默认行为，API Key 规则用于精确覆盖。所有变更仅影响后续请求，不会删除数据库、缓冲区或 Drive 中的历史资料。',
        effective: '当前记录 API Key',
        modeAll: '记录全部',
        modeAllDescription: '默认记录所有 API Key；可单独排除指定 Key。',
        modeSelected: '仅记录选中 Key',
        modeSelectedDescription: '默认不记录，仅记录被明确设为“强制记录”的 Key。',
        modeDisabled: '暂停全部记录',
        modeDisabledDescription: '立即停止所有后续 Session 采集，现有历史资料保持不变。',
        futureOnly: '策略修改只作用于后续请求。暂停或排除不会删除已经保存、待发送或已上传的 Session。',
        searchPlaceholder: '按 Key 名称、ID 或用户邮箱搜索',
        recording: '正在记录',
        notRecording: '不记录',
        owner: '所属用户',
        group: '分组',
        ...
        include: '强制记录',
        exclude: '始终排除',
        onlyThisKey: '只记录此 Key',
        noKeys: '没有匹配的 API Key',
        pagination: '第 {page} / {pages} 页，共 {total} 个 Key',
        confirmModeTitle: '切换为“{mode}”？',
        confirmMode_all: '恢复记录全部 API Key；仍会遵守单 Key 的“始终排除”规则。',
        confirmMode_selected: '切换后，只有明确设为“强制记录”的 API Key 会产生新的 Session 记录。',
        confirmMode_disabled: '切换后，所有 API Key 将立即停止产生新的 Session 记录。历史数据不会被删除。',
        onlyConfirmTitle: '只记录这一把 API Key？',
        onlyConfirmMessage: '将全局切换为“仅记录选中 Key”，清除其他 Key 的覆盖规则，并只保留 #{id} {name}。',
        apply: '应用策略',
        saved: 'Session 采集策略已更新',
        saveFailed: 'Session 采集策略更新失败',
        loadFailed: 'API Key 采集策略加载失败'
      }
    },
