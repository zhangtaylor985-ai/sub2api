export default {
  ...
  keyUsage: {
    ...
    tokenPackages: '流量包',
    stackedPlans: '套餐叠加',
    stackedPlansTitle: '当前有 {count} 个套餐同时生效',
    stackedPlansHint: '每个套餐独立计算周期；套餐到期后，其日限额、周限额和并发贡献会自动移除。',
    overallExpiry: 'API Key 总到期时间',
    effectiveDailyLimit: '当前叠加日限额',
    effectiveWeeklyLimit: '当前叠加周限额',
    effectiveConcurrency: '当前叠加并发',
    planActive: '生效中',
    planUpcoming: '待生效',
    planExpired: '已到期',
    concurrentUnits: '并发',
    ...
    packageRangeUsed: '本范围流量包支付',
    packageHint: '日限额或周限额用完后，超出部分会按需从流量包扣除',
    packageRecentCount: '最近 {count} 条',
    packageUsedPct: '已用 {pct}%',
    packageRecentUsage: '最近流量包消耗',
    packagePaid: '流量包支付',
  ...
  keys: {
    ...
    expirationRequiredHint: '所有 API 密钥都必须设置过期时间；新建时默认 30 天。',
    expirationRequired: '请设置 API 密钥的过期时间',
    expirationInvalid: '过期时间格式无效',
    expirationFutureRequired: '过期时间必须晚于当前时间',
  ...
  admin: {
    ...
    apiKeys: {
      ...
      addPlanPackage: '叠加套餐',
      confirmPlanPackage: '确认叠加套餐',
      currentExpiry: '当前总到期时间',
      effectiveDailyLimit: '当前叠加日限额',
      effectiveWeeklyLimit: '当前叠加周限额',
      effectiveConcurrency: '当前叠加并发',
      planPackageHistory: '套餐周期与生效状态',
      planPeriod: '服务周期',
      planContribution: '额度贡献（日 / 周 / 并发）',
      concurrentUnits: '并发',
      samePlanImmediateHint: '检测到同套餐仍有效：本次套餐立即叠加额度和并发，总到期时间从该套餐现有最晚到期日继续顺延。',
      planImmediateHint: '本次套餐立即生效，并在重叠周期内与其他有效套餐叠加额度和并发。',
      planActive: '生效中',
      planUpcoming: '待生效',
      planExpired: '已到期',
      planPackageAdded: '套餐已叠加',
      planPackageAlreadyAdded: '该次叠加已处理，无需重复操作',
      allStatus: '全部状态',
      allGroups: '全部分组',
      ungrouped: '未绑定分组',
      systemOwner: '系统托管用户',
      ...
      generated: '自动生成',
      weeklyPeriod: '周周期',
      resetDailyWindow: '重置日限额',
      resetWeeklyWindow: '重置周限额',
      resettingWindow: '重置中...',
      currentDailyUsage: '本日已用 ${used}',
      currentWeeklyUsage: '本周已用 ${used}',
      confirmDailyReset: '确认重置 API Key「{name}」的日限额吗？当前日用量 ${used} 将清零，周期仍按北京时间每天 00:00 自然重置；5 小时和周限额不会改变。',
      confirmWeeklyReset: '确认重置 API Key「{name}」的周限额吗？当前周用量 ${used} 将清零，并从现在开始重新计算 7 天，预计于 {end} 结束；日限额和 5 小时限额不会改变。',
      dailyWindowResetSuccess: '日限额已重置',
      weeklyWindowResetSuccess: '周限额已从当前时间重新计算',
      ...
      form: {
        ...
        dispatchOverrideHint: '留空表示依次继承分组和全局配置；可填写 gpt-5.4 或 gpt-5.6-sol，仅影响这把 API Key 的 Claude /v1/messages 到 OpenAI/Codex 调度。',
        allowFable5: '允许 Claude Fable 5',
        allowFable5Hint: '开启后仅这把 API Key 可请求 claude-fable-5，并默认映射到 gpt-5.4。',
        opusMappedModel: 'Opus 目标模型',
        sonnetMappedModel: 'Sonnet 目标模型',
        haikuMappedModel: 'Haiku 目标模型',
        expiresAt: '过期时间',
        expiresInDays: '{days} 天',
        expirationRequiredHint: '必填；新建 API Key 默认 30 天后过期。',
        zeroUnlimited: '0 表示不限',
        zeroInheritGroupLimit: '0 表示继承分组限额',
        zeroInheritConcurrency: '0 表示继承分组 / 用户并发',
        resetQuota: '重置总额度用量',
        resetRateUsage: '重置全部限速窗口用量（5 小时 / 日 / 周）',
        clearExpires: '清空过期时间',
        ...
        planMonths: '购买月数',
        planNote: '付款备注',
        planNotePlaceholder: '例如：8 月 22 日微信收款'
      },
      errors: {
        ...
        loadPlanPackagesFailed: '加载套餐周期失败',
        addPlanPackageFailed: '叠加套餐失败',
        invalidPlanPackage: '请选择有效套餐并填写 1–24 个月',
        invalidRateMultiplier: '倍速必须大于 0',
        expirationRequired: '请设置 API Key 的过期时间',
        expirationFutureRequired: '过期时间必须晚于当前时间',
        invalidExpiresAt: '过期时间格式无效',
        invalidWeeklyWindowStart: '周限额周期起点格式无效',
        invalidWeeklyWindowRange: '周限额周期起点必须在过去 7 天内，且不能晚于当前时间',
        resetWindowFailed: '重置限额窗口失败'
      },
    ...
    groups: {
      ...
      openaiMessages: {
        ...
        allowFable5: '分组开启 Claude Fable 5',
        allowFable5Hint: '开启后，此 OpenAI 分组下未单独覆盖的 API Key 可以请求 claude-fable-5，并默认映射到 gpt-5.4。',
        familyMappingTitle: '系列默认映射',
        familyMappingHint: '当请求命中 Opus、Sonnet、Haiku 系列时，会优先使用这里配置的目标模型；留空表示继承全局默认。',
        opusModel: 'Opus 映射模型',
        opusModelPlaceholder: '留空继承全局，例如: gpt-5.6-sol',
        sonnetModel: 'Sonnet 映射模型',
        sonnetModelPlaceholder: '留空继承全局，例如: gpt-5.6-sol',
        haikuModel: 'Haiku 映射模型',
        haikuModelPlaceholder: '留空使用默认 gpt-5.4-mini',
    ...
    accounts: {
      ...
      externalQuotaGate: {
        title: '外部额度闸门',
        description: '检测到外部消耗后开放账号，到期先排空旧会话再关闭。',
        enable: '启用外部额度闸门',
        disable: '关闭外部额度闸门',
        available: '当前可用',
        unavailable: '当前不可用',
        drainingOnly: '排空中 · 仅旧会话',
        refresh: '立即检查',
        refreshing: '检查中...',
        policyInterval: '每分钟检查',
        policyLease: '确认后开放 {minutes} 分钟',
        policyDrain: '到期后旧会话安全排空',
        grantMinutes: '检测后开放时长（分钟）',
        grantMinutesHint: '每账号可配置 30–720 分钟；修改只影响下一次开放，不改变当前截止时间。',
        savePolicy: '保存时长',
        policySaved: '开放时长已保存',
        primaryUsage: '上游主窗口用量',
        observationBaseline: '关闭观察基线',
        externalDelta: '最近确认外部增量',
        lastCheck: '最近检查',
        externalDetectedAt: '最近外部确认时间',
        leaseUntil: '当前/最近放行截止',
        activeStickySessions: '活跃粘连',
        inflightAndWaiting: '在途 / 等待',
        drainProgress: '排空确认',
        drainChecks: '连续空闲确认 {current}/{required}',
        stickyCountShort: '粘连 {count}',
        recentEvents: '最近状态变化',
        failClosedHint: '开放截止后停止接收新会话，但已有粘连继续使用原账号；活跃粘连、在途请求和等待请求连续两次均为 0 后才真正关闭。',
        disabledHint: '闸门已关闭，账号保持不可调度；重新启用后会先经过 2 分钟安静观察并建立基线。',
        schedulableManaged: '该账号的可调度状态由外部额度闸门自动管理',
        badgeAvailable: '外额放行',
        badgeAvailableTitle: '已确认外部额度消耗，当前处于配置的开放期或旧会话排空期',
        badgeDraining: '外额排空',
        badgeDrainingTitle: '已停止接收新会话，正在保留 {count} 个活跃粘连',
        badgeUnavailable: '外额关闭',
        badgeUnavailableTitle: '外部额度闸门已启用，当前不允许调度',
        badgeError: '外额异常',
        badgeErrorTitle: '外部额度检查异常，已按安全策略保持不可用',
        enabledSuccess: '外部额度闸门已启用并完成首次检查',
        disabledSuccess: '外部额度闸门已关闭，账号已置为不可用',
        refreshedSuccess: '外部额度状态已刷新',
        updateFailed: '更新外部额度闸门失败',
        refreshFailed: '刷新外部额度状态失败',
        decisions: {
          initializing: '等待首次检查',
          externalDecrease: '检测到外部消耗',
          leaseActive: '外部消耗可用期内',
          leaseActiveUpstreamError: '上游检查暂时失败，继续原放行期',
          drainingStarted: '开放到期，开始排空旧会话',
          draining: '正在排空旧会话',
          drainingWithoutLease: '无有效开放期，正在安全排空',
          drainingWindowChanged: '额度窗口变化，正在安全排空',
          drainingUpstreamError: '上游检查异常，继续安全排空',
          drainingCheckError: '排空状态检查异常，暂不关闭',
          drainComplete: '旧会话已排空，账号已关闭',
          upstreamUnavailable: '上游额度不可用',
          baselineCreated: '已建立检测基线',
          observingExternalUsage: '正在观察外部消耗',
          observationCooldown: '正在等待本站流量排空',
          noExternalDecrease: '未检测到外部消耗',
          localTraffic: '检测到本站流量，已重建基线',
          leaseExpired: '外部消耗可用期已结束',
          windowChanged: '额度窗口已变化',
          inactiveLeaseDiscarded: '旧可用期已失效',
          schedulableWithoutLeaseClosed: '无有效放行期，已自动关闭',
          invalidAccount: '账号状态不符合要求',
          upstreamError: '上游额度检查失败',
          localUsageError: '本系统用量检查失败',
          unknown: '状态待确认'
        }
      },
      createAccount: '添加账号',
    ...
    settings: {
      ...
      gatewayForwarding: {
        ...
        openaiMessagesDispatchDefaultTarget: 'Claude → GPT 全局默认目标',
        openaiMessagesDispatchDefaultTargetHint: '用于 OpenAI 分组中未被 API Key 或分组显式覆盖的 Opus/Sonnet 请求。优先级：API Key > 分组 > 此全局设置；Haiku 仍默认使用 gpt-5.4-mini。',
      },
      webSearchEmulation: {
        title: 'Web Search 模拟',
