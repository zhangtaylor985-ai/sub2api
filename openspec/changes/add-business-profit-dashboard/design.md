...
经营口径已经确定：不做未来预测；当前收入由有效计费 API Key 与有效私下客户订阅组成；两项菲律宾订阅账号各为 USD 150/月；当前 USD/CNY 为 6.75；历史月份使用各自锁定汇率。私下客户订阅必须计入，但与 API Key 显式关联时不得重复。
...
- 让价格、Key 排除/覆盖、客户订阅关联、成本和汇率成为结构化数据，而不是临时 SQL 常量。
...
### 2. 收入解析使用“规则 + Key 配置 + 显式关联”的优先级

API Key 的基础价格由启用的 `business_pricing_rules` 按 `group_id` 解析；`business_api_key_configs` 可排除某个 Key、覆盖金额，或关联一个私下客户订阅。解析顺序为：

1. Key 配置为排除时不计收入。
2. Key 显式关联有效客户订阅时，以客户订阅金额/到期日计一次，Key 基础价格不再重复计入。
3. Key 有金额覆盖时使用覆盖值。
4. 否则使用分组价格规则。
5. 未关联任何 Key 的有效客户订阅作为独立收入计入。

不按名称自动建立关联；名称只用于生成对账建议。
...
当前或锁账月份计算成本时，将原币金额与所用汇率一起复制进快照明细。历史不跟随汇率配置变化。免费账号作为零金额成本记录保留，既能解释账号成本结构，也不会影响利润。

### 6. 利润公式保持清晰且可审计

- 总收入 = API Key 收入 + 独立/关联客户订阅收入。
- 毛利 = 总收入 - 直接成本。
- 净利润 = 毛利 - 运营费用。
...
- [客户订阅与 API Key 可能代表同一客户] → 只通过 ID 显式关联去重，并在对账页提示疑似同名/日期差异。
...
4. 应用 migration 后初始化四档价格、6个排除 Key、4个免费账号、2个 USD 150 付费账号及当月 USD/CNY 6.75。
5. 对账当前 API Key 与客户订阅，确认没有误关联后再生成首个当前月快照；不上线未来预测数据。
6. 回滚时恢复旧 binary；新增表不影响请求路径，可保留数据。若必须完全回滚 schema，使用备份恢复而不是手工删除生产表。

## Open Questions

无阻塞问题。服务器、代理、域名等未知成本在成本菜单上线后由管理员补录；它们缺失时页面必须明确显示“成本未完整”。
