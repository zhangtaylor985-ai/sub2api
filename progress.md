...
- 2026-08-13：最终内容级抽检确认 Claude Code 主记录的交付 request/response model 均为 `claude-opus-5`，thinking block 签名非空；GPT model、upstream、mapping、account 字段均未进入交付投影。Codex 真实主记录已入库且 `deliverable=true`。
- 2026-08-13：Codex 交付抽检发现客户端把 AGENTS/runtime 启动包标成 user 内容；新增仅针对同时命中多项稳定启动标记的过滤，原始审计信封仍完整、真实用户消息保留。全量 Go、Session race 与真实 Codex 返回门禁通过后，app 更新为 SHA256=`3149aa2031345c3965391cf83d1cd7e3333352f666c15a6f38a3212777277b5d`。
- 2026-08-13：高峰期连续 100 MiB 级会话使 spool 接近 2 GiB。新增满载前置容量门禁：HTTP 与 Codex WS 都在捕获前检查容量，满载时继续服务用户但跳过新 Session，低于上限后自动恢复；app 最终容量保护 SHA256=`2f5f741f9d9f74d96d622d4b24a584603e8a4802cb5270b1f3b77280f95777fa`。
- 2026-08-13：forwarder 增加多 endpoint worker 分流，受限 SSH 通道扩到 16 条并按 1 秒错开连接；首次 16 条同时握手触发中继 `MaxStartups`，配置自动恢复 8 条，错开后 16 条全部健康。
- 2026-08-13：定位接收端小时 advisory lock 把 16 路摄取串行为 1 路；改为摄取共享锁、导出/清理排他锁，并新增冻结互斥集成用例。首次误把 ARM64 sessiond 部署到 AMD64 腾讯机，systemd 以 `Exec format error` 拒绝；约 25 秒后从精确备份恢复健康，spool 无丢失，随后部署通过架构/SHA 校验的 AMD64 sessiond `a639fcdf4bc69c3d878a377ced133eff070ee298a7770d0b48d258b40f26b9da`。
- 2026-08-13：8 分钟停滞阈值会误杀 100 MiB 在途上传，09:41 曾触发一次；阈值已提高为 30 分钟，只更新 watchdog 脚本，未中断随后正在运行的通道。
- 2026-08-13：为避免 16 个大压缩包同时结束上传后在 2 GiB 隔离机并发解压触发 OOM，新增独立解码并发门禁；定向测试、Session race 和全量 `go test ./... -count=1` 均通过。腾讯机部署 AMD64 sessiond SHA256=`a72eee7daea80a5b9ef349ef764335f2e7681d8ce1354d69402aef18ac6416a2`，显式配置 `SESSION_INGEST_MAX_DECODE_CONCURRENT=1`；旧二进制备份 `/opt/sub2api/sessiond.bak.20260813T100108Z-before-bounded-decode`，旧配置备份 `/etc/sub2api/session-delivery.env.bak.20260813T100108Z-before-bounded-decode`。发布后 health 正常、NRestarts=0、16 条连接自动恢复，spool 未删除。
- 2026-08-13：真实 Codex 内容抽检进一步发现 runtime `<environment_context>` 与用户提示位于同一 user message 的不同 part；修复为仅移除完整机器包装 part，保留真实提示与自然语言标签引用。协议专项、Session race、全量 Go 和真实 Codex canary 均通过；最终记录 `rec_05d305f95a979db65fa4d5f920f2f502` 的 request/response model 均为 `claude-opus-5`，thinking signature 非空，Codex/OpenAI/GPT/upstream/mapping/account/runtime 标记全部为 false。
- 2026-08-13：首次本机 `18080` SSH 转发因端口已被既有进程占用而未建立，前两次 Codex 实际命中本机旧服务并返回 413；确认远端 canary 无访问日志后改用本机 `28080 -> 远端 18080`，响应与远端日志双重确认命中。该错误未触达正式 8080，也未修改既有本机进程。
- 2026-08-13：最终 ARM64 app SHA256=`a961fe5160c5f78cb634ba4716fe4d22a082a08f94a78a76a9f4800cfc7997b5` 先在远端 18080 canary 通过 health、管理页面、认证门禁和真实 Codex，再切正式服务；旧版备份 `/opt/sub2api/sub2api.bak.20260813T102616Z-before-codex-context-part-filter`。正式本地/公网 health 正常，管理页面 200、未认证 overview 401、NRestarts=0。
- 2026-08-13：watchdog 的 `sort | head` 在大 pending 列表产生无害 Broken pipe 日志；改为完整消费输入的 `sort | sed -n '1,16p'`，线上脚本 SHA256=`0ac7a8f97eb49aa1a4d3cf58264cb4542485d6343673926931a475f41ab78372`，备份 `/opt/sub2api/session-tunnel-health.bak.20260813T102831Z-before-sed-fingerprint`，手动与 timer 执行均 success 且未重启隧道。
