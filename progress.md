...
- 已完整阅读 `planning-with-files`，catchup 不支持 Codex 原生 session，改用 Git 状态和现有规划文件恢复。
- 发现主工作树存在大量未提交经营管理改动；已 fetch `origin`，从 `origin/main=4a546ce451a8cae1078c15f3d89b1842fc0507b5` 创建独立分支 `codex/external-quota-gate-v2` 和干净 worktree。
- 首次追加规划文件时上下文不匹配，补丁未写入；已改用真实文件末尾精确追加并记录错误。
- 已建立本次任务的阶段计划、调查结论和风险边界；开始审计现有状态机、测试与前端展示。
- 已完成后端第一轮审计：确认现实现只比较相邻一分钟、遇本站流量即推进基线、租约仅 10 分钟且到期覆盖历史。
- 已确定兼容重构方向：60 分钟固定租约、2 分钟关闭观察安静期、跨轮询累计外部差值、旧状态安全重建、JSON extra 扩展无需迁移。
- 已完成前端与 API 审计：现有专用启停/刷新 API 可复用，账号 extra 可直接承载新增状态；列表徽标保持兼容，详情面板需要补充基线、确认时间、最近截止与事件历史。
- 阶段 1 完成，进入后端状态机与单元测试实现。
- 已完成后端状态结构和主判断链路第一版：默认租约60分钟、安静期2分钟、关闭基线累计比较、放行不续期、最近12个状态事件。
- 首轮定向测试成功编译，但3个旧测试因缺少新观察基线而按兼容逻辑返回 `baseline_created`；这是测试夹具需要迁移，不是编译错误，已记录并开始更新测试。
- 测试迁移过程中发现一处局部变量重复声明和一处仍为可调度状态的旧夹具；已按真实关闭观察前提修正。
- 后端定向状态机测试已通过；新增覆盖关闭累计差值、2分钟安静期、60分钟固定截止、不续期、无租约脏状态自动关闭、默认参数和12条事件上限。
- 前端已完成新状态字段、策略标签、观察基线、最近确认时间、历史截止和状态事件列表；组件3项 Vitest、相关 ESLint 与 vue-tsc 均通过。
- `go test ./... -count=1` 后端全量通过；前端 `npm run build` 生产构建通过。
- 完整前端 Vitest 为107个文件通过、5个既有文件失败，合计676通过/12失败；失败数量与既有基线一致，本功能组件3项全部通过。
- 已完整阅读 `frontend-design` 并按既有后台约束复核 UI：采用精密运维控制台的信息层级与状态色，不引入额外视觉体系。
- 已删除本次创建的临时 `frontend/node_modules` 链接；未安装或更新依赖。
- 阶段2–4完成，进入最终 diff、安全边界和 Git 状态审查。
- 最终状态机补充“活动放行期内临时上游查询失败不提前关闭”语义，新增专项测试并通过；原固定截止时间保持不变。
- 最终后端 `go test ./... -count=1` 再次全量通过，前端 `npm run build` 再次通过，`git diff --check` 通过。
- 最终 diff 只包含闸门后端、相关测试、管理 UI/类型/中英文文案和本任务规划记录；无 migration、无依赖变更、无生产写入。
- 阶段5完成，准备规范化本地提交；生产发布继续等待单独确认。

---

# 2026-08-20 外部额度闸门会话安全排空

- 用户确认采用“默认开放2小时且可配置；到期停止新会话；已有粘连自然排空后再关闭”的策略。
- 已说明本次将修改OpenAI调度、Redis粘连索引、额度闸门状态机和管理UI，并明确本轮不部署生产。
- 已完整阅读 `planning-with-files`；catchup不支持Codex原生session，使用Git状态和既有规划文件恢复。
- 当前隔离分支为 `codex/external-quota-gate-v2`，HEAD=`b58ad9cd3`，相对 `origin/main` 仅包含上一版一小时固定放行提交，工作树初始干净。
- 只读源码核对确认：不可调度会清理OpenAI粘连；默认粘连TTL为60分钟并滚动刷新；现有反向活跃会话计数只覆盖Anthropic，OpenAI缺少按账号反查索引。
- 已建立本次6阶段计划，开始实现会话反向索引与排空调度语义。
- 已在GatewayCache的OpenAI粘连写入、刷新、删除路径同步维护账号级Sorted Set反向索引；索引使用Redis服务器时间记录绑定过期点。
- 反向索引覆盖 `session_hash` 和 `previous_response_id` 两种续链；账号重绑会移除旧账号索引，显式删除会立即清理。
- repository/service定向编译测试通过；Redis生命周期集成用例已新增，等待具备integration环境时执行。
- OpenAI高级调度已验证：排空账号继续命中已有session粘连，但全新session会选择其他账号。
- 额度闸门后端已改为默认120分钟，并支持账号级30–720分钟配置；修改配置不重置当前已生效的固定截止时间。
- 租约到期、窗口变化或无有效租约时不再直接关闭，而是进入排空；连续两轮粘连/并发/等待均为0后才关闭并建立新观察基线。
- Redis反向索引或并发统计异常时保持排空且不关闭；新增活跃粘连、两轮空闲、统计异常和自定义时长测试，定向测试通过。
- 启用闸门时若账号已在调度，改为先排空旧会话；已关闭账号仍立即建立外部观察基线。
- 管理UI已增加30–720分钟账号级配置、排空专用状态、活跃粘连、在途/等待和连续空闲确认进度；策略视觉沿用现有精密运维控制台的状态色和紧凑层级。
- 前端组件专项5项测试与 `vue-tsc --noEmit` 通过；后端handler/routes编译通过。
- `cmd/server` 首次编译发现Wire生成文件参数过期；已按项目命令重新生成，待复编译确认。
- Wire刷新后 `cmd/server` 编译通过；后端 `go test ./... -count=1` 全量通过。
- GatewayCache在真实Redis integration路径通过，覆盖OpenAI粘连新增、续期、换绑、删除、过期清理与索引TTL保护。
- 前端 `lint:check`、`typecheck`、生产构建通过；闸门组件专项5项通过。全量Vitest为678项通过、12项既有基线失败，失败文件与任务前基线一致。
